...
`prism-mob-vault-index` is centered on mobile workflows work in Go. The technical target is: Create a Go reference implementation for vault workflows, centered on state machine modeling, transition tables, and invalid-transition tests.
...
The score is deterministic. The current threshold is 165. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 5.
